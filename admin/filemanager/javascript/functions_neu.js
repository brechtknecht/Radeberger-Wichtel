...
function insertForm(){
	...
	var uploader = "";
	uploader+= "<form id=\"uploader\" action=\"upload.php" + window.location.search + "\" method=\"post\" enctype=\"multipart/form-data\" style=\"width: 750px; height: 508px; background-color: #FFFFFF;\">";
	uploader+= "<input type=\"hidden\" name=\"uploader\" value=\"form\" />";
	uploader+= "<fieldset>";
	uploader+= "<legend>Datei(en) auswählen</legend>";
	for(var i = 0; i < 10; i++){
		//uploader+= "<label for=\"file_" + i + "\">Datei " + (i+1) + "</label>";
		uploader+= "<input type=\"file\" name=\"file_" + i + "\" value=\"\" />";		
	}
	
	uploader+= "</fieldset>";
	uploader+= "<fieldset>";
	uploader+= "<legend>Aktionen</legend>";
	uploader+= "<button type=\"submit\">Datei(en) übertragen</button>";
	uploader+= "<button type=\"button\" onclick=\"this.parentNode.parentNode.parentNode.removeChild(this.parentNode.parentNode)\">Abbrechen</button>";		
...
function insertFlash(url){
    ...
    uploader+= " data=\"" + url + "&uscript="+ rootURL +"upload.php\"";
    uploader+= " width=\"750\"";
    uploader+= " height=\"508\"";
    uploader+= " type=\"application/x-shockwave-flash\"";
    uploader+= ">";
    uploader+= "<param name=\"movie\" value=\"" + url + "&uscript=upload.php\"></param>";
    uploader+= "</object>";
    document.getElementById("uploadContainer").innerHTML = uploader;
    return false;
}
...
function showFileDesc(fileObj){
    ...
    if(langArray && langArray.length > 0){
		...
		for(var i = 0; i < langArray.length; i++){
			outStr+= "<fieldset class=\"fileDesc\">";
