...
function db_connect($nodie=0){
 ...
 if (!$dbh) {
    $err_msg='Cannot connect to the database because: '.mysqli_error($_SESSION['conn']);
 ...
 if ($dbh && $DB['db']) {
  ...
  if (!$res) {
     $err_msg='Cannot select db because: '.mysqli_error($_SESSION['conn']);
     if (!$nodie) die($err_msg);
  }else{
...
function dbq($s){
 ...
 return "'".mysqli_real_escape_string($_SESSION['conn'], $s,$dbh)."'";
}
