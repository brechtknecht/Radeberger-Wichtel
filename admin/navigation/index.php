...
isLoggedIn();
function NAVI_moduleNavigation($dir = NULL, $output_str = NULL,$check_perm = true){
	if($dir != NULL && is_dir($dir)){
		...
		while($module=readdir($fp)){
			if(isset($_SESSION['cms_user']['modules'])){
				$array_keys = array_keys($_SESSION['cms_user']['modules']);
			}
			if($check_perm == true){
				if($_SESSION['cms_user']['user_role']=="Administrator" || (isset($array_keys) && in_array($module,$array_keys))){
					if(is_dir($dir.$module) && $module!=".." && $module!="." && file_exists($dir.$module."/index.php")){
						...
						if(is_dir($dir.$module."/submodules") && !isset($_SESSION['cms_user']['modules'])){
							
							$output_str.= NAVI_moduleNavigation($dir.$module."/submodules/","",false);
						}
						$output_str.= "</li>";
					}
				}
