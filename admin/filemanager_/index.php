...
foreach($_SESSION['lang_array'] as $val){
	$query.= " (SELECT description FROM _cms_hp_files_desc_ t2 WHERE t2.entry_parent_id=t1.entry_id AND language='".mysqli_real_escape_string($_SESSION['conn'], $val)."' LIMIT 1) AS desc_".htmlspecialchars($val);
}
...
foreach($_SESSION['lang_array'] as $val){
	?>
	langArray.push("<?php echo htmlspecialchars($val);?>");
	<?php
}
?>
</script>
...
<body>
	...
	<div id="fileContent">
		...
		*/
		?>
        <ul>
		    <li><button type="button" onclick="addFile()">Datei(en) hinzufügen</button></li>
